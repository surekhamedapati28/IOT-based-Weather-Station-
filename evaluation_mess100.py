...
message_limit = 100  # Total number of messages to send
...
# Main Loop with 100 Message Limit
...
while messages_sent < message_limit:
    try:
        ...
        # Periodically Calculate Message Loss
        if messages_sent % 10 == 0 and messages_sent > 0:
            calculate_message_loss()
        
        # Delay
        utime.sleep(1)  # 1-second delay for faster sending
    except Exception as e:
        print(f"Error in main loop: {e}")

# End Program after 100 Messages
print("Message Limit Reached: 100 Messages Sent")
calculate_message_loss()
elapsed_time = time.time() - start_time
throughput = messages_sent / elapsed_time
print(f"Final Throughput: {throughput:.2f} messages/second")
